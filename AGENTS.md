...
- `TurnController` shares ONE `AVAudioEngine` between `mic` (`MicCapture`) and `player` (`AudioPlayer`), with Voice-Processing I/O enabled on both nodes for AEC (`player`'s TTS/backchannel output gets cancelled out of `mic`'s input). `MicCapture(engine:)` with an external engine never calls `engine.start()`/`.stop()` itself — see the comment on `ownsEngine` — that's deliberately left to whoever else uses the engine (`AudioPlayer`). `bootstrap()` starts it once at launch, and nothing stops it again until the app quits (`shutdown()` → `player.stop()`).
- Aha (2026-08-21): first tried making this lazy — start on first `beginCapture()`, release via `player.stop()` once idle, to fix the mic-in-use indicator staying lit for the app's whole lifetime — and it broke recording outright. Initially suspected a stop()→start() restart problem with Voice-Processing I/O; **that diagnosis was wrong**. The real cause, found via Console (`log show --predicate 'subsystem == "ai.oasis.echo.mac"'`, note `log` is shadowed by a zsh builtin — use `/usr/bin/log` or `command log`): Voice-Processing I/O (AUVoiceIO) can fail `kAUInitialize` on the very FIRST ever `engine.start()`, no restart involved, in some audio-device configurations — observed here with an external DisplayPort monitor's audio output and a virtual "Microsoft Teams Audio" device both present alongside the built-in mic/speakers (`system_profiler SPAudioDataType` to inspect). `try? player.prepare()` swallowed that error silently, so the shared engine simply never started: no tap ever produced buffers, no OS mic indicator, "no mic detected" from every capture attempt regardless of hotkey state.
- Fixed with a fallback, not a revert of the lazy-release idea (which was reverted separately and is still parked — see below): `bootstrap()` now retries `player.prepare()` once with `setVoiceProcessingEnabled(false)` on both nodes if the first attempt throws. Verified live: retry log showed `AudioPlayer ready (output=48000.000000 Hz)` immediately after the VPIO failure. Losing AEC on an audio setup where VPIO won't initialize is far better than losing capture entirely.
- The lazy-start/idle-release approach for the mic-in-use indicator is still shelved pending real-hardware verification of repeated start→stop→start cycles specifically WITH Voice-Processing enabled (this session's single failure was a first-start failure, not conclusively a restart failure, so the original suspicion is unconfirmed either way) — or sidestep the restart question by using a separate plain (non-VPIO) engine for capture that stops/starts freely like `MeetingController`'s already does.
...
- Aha (2026-08-21): `analyzeDiff` (packages/coordinator/src/postprocess/corrections.ts) used to only extract a word-level rule when original/corrected had the EXACT SAME token count with exactly one mismatch — so any replacement that changes the token count (e.g. "P, N, L" collapsing into "PNL") could never qualify, and fell back to indexing the WHOLE sentence as a phrase, which then essentially never matches anything else verbatim. Rewritten to trim the longest common prefix/suffix (case-insensitive, token-wise) and promote whatever's left in the middle, on EITHER side, as the rule — works regardless of token-count changes. Span capped at 6 words per side (longer = treated as an unrelated rewrite, phrase-only); edge punctuation stripped so the extracted phrase matches cleanly against `RuleStage`'s `\b`-anchored regex (a trailing comma left in place means the boundary check right after it never fires — non-word char followed by non-word char is not a `\b` transition). `CorrectionStore.addCorrection` now also skips adding the whole-sentence phrase when a targeted pair WAS found (no point keeping both — the phrase is strictly worse for generalizing). `POST /correction`'s `addedAsPhrase` response field reports whether the phrase actually landed in the store (`correctionStore.phrases().includes(...)`), not the analyzer's raw recommendation — those diverged once the skip-when-pair-found behavior shipped.
- Aha (2026-08-21): added `PostPasteWatcher` (apps/mac/Sources/OasisEcho/Paste/PostPasteWatcher.swift) — after a Transcribe-mode paste, polls the target app's focused element for edits the user makes themselves and routes a settled diff into the same correction-review bubble, no manual copy/menu action needed. Two real findings from getting this working against Claude Desktop (Electron):
  1. The obvious approach — `AXUIElementCreateSystemWide()` + `kAXFocusedApplicationAttribute` ("who's focused, system-wide?") — consistently failed with AXError -25212 (`kAXErrorNoValue`), apparently because OasisEcho (an accessory app with no windows) had just driven Claude Desktop frontmost via synthetic activation rather than a real user click, and the system's AX-focus tracking didn't resolve from that state. Fix: skip that question entirely — we already know the PID we pasted into, so build its AXUIElement directly via `AXUIElementCreateApplication(targetPID)` and ask IT which of its own elements is focused.
  2. Even asking the app directly still got -25212 at first. Chromium/Electron computes its accessibility tree LAZILY — a caller asking for `kAXFocusedUIElement` before that's happened gets exactly this error, not because nothing is focused but because the tree was never built. Setting `AXManualAccessibility` / `AXEnhancedUserInterface` (both, since different Electron/Chromium versions respond to different ones) to `kCFBooleanTrue` on the app element is the documented way to force activation — and it can take ~2s the first time, so this needs the retry loop (already had one for a different reason) rather than a single attempt.
  - Debugging note: `log` is shadowed by a zsh builtin — use `command log` or `/usr/bin/log`. `log show` (archived query) can lag confusingly after a fresh launch; `log stream` (live) is more reliable when actively testing against a process that just started.
